...
class KeyValueStore:
    ...
    def handle_client(self, client_socket, client_address):
        while True:
            ...
            if command.startswith("login"):
                ...
                self.handle_login(client_socket, client_address, username, password)
    ...
    def handle_login(self, client_socket, client_address, username, password):
        if username in self.users:
            if self.users[username]["password"] == password:
                role = self.clients[username].get("role")
                self.clients[username]["socket"] = client_socket
                self.clients[username]["address"] = client_address
                client_socket.send(
                    f"\nLogin successful as {role.upper()}.".encode("utf-8")
                )
                print(f"User {username} logged in as {role.upper()}.")
            else:
                client_socket.send(
                    "\nIncorrect password. Please enter the correct password.".encode(
                        "utf-8"
                    )
                )
        else:
            client_socket.send(
                "\nUsername does not exist. Please signup first.".encode("utf-8")
            )
    ...
    def handle_signup(self, client_socket, client_address, username, password):
        if username in self.users:
            ...
            print(f"User {username} signed up and logged in as GUEST.")
    ...
    def handle_put(self, client_socket, key, value):
        ...
        print(f"User {username} performed PUT operation.")
    ...
    def handle_superput(self, client_socket, target_user, key, value):
        ...
        if target_user in self.client_data:
            ...
            print(f"User {username} performed SUPERPUT operation on {target_user}.")
    ...
    def handle_superget(self, client_socket, target_user, key):
        ...
        if target_user in self.client_data and key in self.client_data[target_user]:
            ...
            print(f"User {username} performed SUPERGET operation on {target_user}.")
        elif target_user not in self.users:
            client_socket.send(f'\nInvalid target user "{target_user}"'.encode("utf-8"))
        else:
            client_socket.send(
    ...
    def handle_view_all_users(self, client_socket):
        ...
        print(f"User {username} viewed all users registered in the server.")
